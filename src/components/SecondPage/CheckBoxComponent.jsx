...
const DepartmentListComponent = () => {
  ...
  const handleDepartmentSelect = (departmentId) => {
    if (selectedDepartments.includes(departmentId)) {
      ...
      // Select the department and all its sub-departments
      const selectedDepartment = departmentData.find((department) => department.id === departmentId);
      if (selectedDepartment) {
        const subDepartmentIds = selectedDepartment.subDepartments.map((subDepartment) => subDepartment.id);
        setSelectedDepartments((prevState) => Array.from(new Set([...prevState, departmentId, ...subDepartmentIds])));
      }
    }
  };
  ...
  const handleSubDepartmentSelect = (subDepartmentId) => {
    ...
    if (allSubSelected) {
      // Unselect all sub-departments
      setSelectedDepartments((prevState) =>
        prevState.filter((id) => !departmentWithSub.subDepartments.some((sub) => sub.id === id))
      );

      // Unselect the department
      setSelectedDepartments((prevState) => prevState.filter((id) => id !== departmentWithSub.id));
    } else {
      // Select all sub-departments
      setSelectedDepartments((prevState) =>
        Array.from(new Set([...prevState, subDepartmentId, departmentWithSub.id]))
      );
    }
  };
  ...
  const isSubDepartmentSelected = (subDepartmentId) => selectedDepartments.includes(subDepartmentId);

  const isDepartmentExpanded = (departmentId) => expandedDepartments.includes(departmentId);

  const getSubDepartmentIndent = (depth) => `${16 * depth}px`; // 16px is the default padding-left for MUI ListItem
  ...
  return (
    <div style={{ height: "100vh", width: "fit-content", display: "flex", flexDirection: "column" }}>
      ...
      {sidebarOpen && (
        <div style={{ width: "fit-content", overflowY: "auto", padding: "8px" }}>
          <List>
            {departmentData.map((department) => (
              <div key={department.id}>
                <ListItem disableGutters sx={{ paddingLeft: getSubDepartmentIndent(0) }}>
                  <Checkbox
                    checked={isDepartmentSelected(department.id)}
                    onChange={() => handleDepartmentSelect(department.id)}
                  />
                  ...
                  <List>
                    {department.subDepartments.map((subDepartment) => (
                      <ListItem
                        key={subDepartment.id}
                        disableGutters
                        sx={{ paddingLeft: getSubDepartmentIndent(1) }}
                      >
                        <Checkbox
                          checked={isSubDepartmentSelected(subDepartment.id)}
                          onChange={() => handleSubDepartmentSelect(subDepartment.id)}
                        />
                        <ListItemText primary={subDepartment.name} />
                      </ListItem>
                    ))}
